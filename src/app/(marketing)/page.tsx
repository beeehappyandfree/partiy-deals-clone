  ...
  return (
    <>
      ...
      flex items-center justify-center text-center text-balance flex-col gap-8 px-4">
        ...
        tracking-tight m-4">Price Smarter, Sell bigger!</h1>
        <p className="text-lg lg:text-3xl max-w-screen-xl">Optimize your product pricing across countries to maximize sales. Capture 85% of the untapped market with location-based dynamic pricing.</p>
        <SignUpButton>
      ...
      <section className="bg-primary text-primary-foreground">
        <div className="container py-16 flex flex-col gap-16 px-8 md:px-15">
      ...
      <section id="pricing" className="px-8 py-16 bg-accent/5">
        <h2 className="text-4xl text-center text-balance font-semibold mb-8">
          Pricing software which pays for itselfs 20x over
        </h2>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{formatCompactNumber(maxNumberOfVisits)} visits per month</CardDescription>
      </CardHeader>
      <CardContent>
        <SignUpButton>
      ...
      <CardFooter className="flex flex-col gap-4 items-start">
        ...
        {canAccessAnalytics && <Feature>
          Access Analytics
        </Feature>}
        {canCustomizeBanner && <Feature>
          Banner Customization
        </Feature>}
        {canRemoveBranding && <Feature>
          Remove Easy PPP Branding
        </Feature>}
      </CardFooter>
    </Card>
  )
...
function Feature({children, className}: {children: React.ReactNode, className?: string}) {
  return (
    <div className={cn("flex items-center gap-2 text-sm", className)}>
      <CheckIcon className="size-4 stroke-accent bg-accent bg-accent/25 rounded-full p-0.5" />
      <span>{children}</span>
    </div>
  )
}
